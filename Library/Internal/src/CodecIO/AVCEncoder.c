...
extern "C" {
    ...
    EncodeAVC *InitAVCEncoder(void) {
        ...
        if (Enc == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, "Not enough memory to allocate EncodeAVC");
        } else {
        ...
        if (Enc == NULL) {
            ...
            if (Enc->PPS->EntropyCodingMode == Arithmetic) {
                while (more_rbsp_trailing_data()) {
                    uint16_t CABACZeroWord = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16); /* equal to 0x0000 */
                }
            }
        }
    }
