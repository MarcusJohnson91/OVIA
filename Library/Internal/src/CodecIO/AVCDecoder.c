...
extern "C" {
    ...
    /*
     ...
     Log(LOG_INFO, "ParseAVCFile", "Parsing AVC File...\n");
     
     // Found a start code.
     if (ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32) == AVCMagic && GetBitBufferPosition(BitB) == 0) {
     ...
     Dec->NAL->NALRefIDC               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2); // 0
     Dec->NAL->NALUnitType             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5); // 6
     ...
     Dec->NAL->AVC3DExtensionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
     } else {
     Dec->NAL->SVCExtensionFlag   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
     }
     ...
     uint32_t NALID = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
     if (NALID == 0x000003) {
     uint8_t NALID2 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
    ...
    /*
     ...
     uint32_t ByteAlignment = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
     if (ByteAlignment == 0x00000001) { // Bytestream is aligned
     uint32_t StreamType = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
     ...
     if ((ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24) == 0x000000) && (IsStreamByteAligned(BitB->BitsUnavailable, 1) == true)) {
    ...
    DecodeAVC *InitAVCDecoder(void) {
        ...
        if (Dec == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, "Pointer to DecodeAVC is NULL");
        } else {
    ...
    int8_t MacroBlock2SliceGroupMap(DecodeAVC *Dec, uint8_t CurrentMacroBlock) { // MbToSliceGroupMap
        ...
        Log(Severity_DEBUG, PlatformIO_FunctionName, "SliceGroups: %hhu, SliceGroupMapType: %d", Dec->PPS->SliceGroups, Dec->PPS->SliceGroupMapType);
        return -1; // failure
    }
    ...
    void rbsp_slice_trailing_bits(DecodeAVC *Dec, BitBuffer *BitB) {
        ...
        if (Dec->PPS->EntropyCodingMode == Arithmetic) {
            while (more_rbsp_trailing_data()) {
                uint64_t CABACZeroWord = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16); /* equal to 0x0000 */
            }
        }
    }
    ...
    void AcquisitionElement3DV(DecodeAVC *Dec, BitBuffer *BitB, uint8_t numViews, uint8_t DeltaFlag, uint8_t PredDirection, uint8_t precMode, uint8_t expLen, uint8_t OutSign[MVCMaxViews], uint8_t OutExp, uint8_t OutMantissa, uint8_t OutManLen[MVCMaxViews]) { // 3dv_acquisition_element
        ...
        if (numViews - DeltaFlag > 1) {
            element_equal_flag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
        ...
        for (uint8_t Value = 0; Value < numValues; Value++) {
            if ((PredDirection == 2) && (Value = 0)) {
                if (precMode == 0) {
                    matissa_len_minus1 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5) + 1;
                    OutManLen[index, Value] = manLen = matissa_len_minus1;
                } else {
                    prec = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5);
                }
            }
            if (PredDirection == 2) {
                sign0 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                mantissa0 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, manLen);
                OutMantissa[index, Value] = mantissa0;
            } else {
                skip_flag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (skip_flag == 0) {
                    sign1 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    OutSign[index, Value] = sign1;
                    exponent_skip_flag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    if (exponent_skip_flag == 0) {
                        exponent1 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, expLen);
    ...
    uint64_t DepthRanges(BitBuffer *BitB, uint64_t NumberOfViews, bool PredDirection, uint64_t Index) { // depth_ranges
        bool NearFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        bool FarFlag  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void RefPicListMVCMod(DecodeAVC *Dec, BitBuffer *BitB) { // ref_pic_list_mvc_modification
        if (((Dec->Slice->Type % 5) != 2) && ((Dec->Slice->Type % 5) != 4)) {
            Dec->Slice->RefPicListModFlag[0] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        ...
        if ((Dec->Slice->Type % 5) == 1) {
            Dec->Slice->RefPicListModFlag[1] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void pred_weight_table(DecodeAVC *Dec, BitBuffer *BitB) { // pred_weight_table
        ...
        for (uint8_t i = 0; i <= Dec->MacroBlock->NumRefIndexActiveLevel0; i++) {
            Dec->Slice->LumaWeightFlag[0] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            if (Dec->SPS->ChromaArrayType != ChromaBW) {
                Dec->Slice->ChromaWeightFlag[0] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        ...
        if ((Dec->Slice->Type % 5) == 1) {
            for (uint8_t i = 0; i <= Dec->MacroBlock->NumRefIndexActiveLevel1; i++) {
                Dec->Slice->LumaWeightFlag[1] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if (Dec->SPS->ChromaArrayType != ChromaBW) {
                    Dec->Slice->ChromaWeightFlag[1] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void DecodeRefPicMarking(DecodeAVC *Dec, BitBuffer *BitB) { // dec_ref_pic_marking
        if (Dec->Slice->SliceIsIDR == true) {
            Dec->NAL->EmptyPictureBufferBeforeDisplay = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->FrameIsLongTermReference        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        } else {
            Dec->NAL->AdaptiveRefPicMarkingModeFlag   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        ...
        if (TotalCoeff(coeff_token) > 0) {
            ...
            for (uint64_t Coefficent = 0; Coefficent < TotalCoeff(coeff_token); Coefficent++) {
                if (Coefficent < TrailingOnes(coeff_token)) {
                    trailing_ones_sign_flag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    ...
                    if ((suffixLength > 0) || (level_prefix >= 14)) {
                        level_suffix = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 0);
    ...
    void ScanNALUnits(DecodeAVC *Dec, BitBuffer *BitB) {
        switch (Dec->NAL->NALUnitType) { // nal_unit_type
            ...
            case NAL_MVCDepthView: // 21
                ...
                break;
            default:
                Log(Severity_DEBUG, PlatformIO_FunctionName, "NAL ID 0x%X is not supported, seeking to next NAL\n", Dec->NAL->NALUnitType);
                break;
        }
    }
    ...
    void rbsp_trailing_bits(DecodeAVC *Dec, BitBuffer *BitB) { // rbsp_trailing_bits
        ...
        rbsp_stop_one_bit = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSequenceParameterSetData(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_data
        if (Dec == NULL) {
            ...
            ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
            Dec->SPS->ProfileIDC                                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8); // 100
            Dec->SPS->ConstraintFlag0                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // 0
            Dec->SPS->ConstraintFlag1                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // 0
            Dec->SPS->ConstraintFlag2                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // 0
            Dec->SPS->ConstraintFlag3                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // 0
            Dec->SPS->ConstraintFlag4                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // 0
            Dec->SPS->ConstraintFlag5                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // 0
            BitBuffer_Seek(BitB, 2); // Zero bits.
            Dec->SPS->LevelIDC[0]                                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8); // 51
            ...
            if ((Dec->SPS->ProfileIDC == 44)  ||
                ...
                if (Dec->SPS->ChromaFormatIDC == Chroma444) {
                    Dec->SPS->SeperateColorPlane                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                ...
                Dec->SPS->QPPrimeBypassFlag                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // qpprime_y_zero_transform_bypass_flag
                Dec->SPS->ScalingMatrixFlag                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);

                if (Dec->SPS->ScalingMatrixFlag == true) {
                    for (uint8_t i = 0; i < ((Dec->SPS->ChromaFormatIDC != Chroma444) ? 8 : 12); i++) {
                        Dec->SPS->ScalingListFlag[i]                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            if (Dec->SPS->PicOrderCount == 0) {
                ...
                Dec->SPS->DeltaPicOrder                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->SPS->GapsInFrameNumAllowed                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->SPS->OnlyMacroBlocksInFrame                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SPS->OnlyMacroBlocksInFrame == false) {
                Dec->SPS->AdaptiveFrameFlag                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->SPS->Inference8x8                                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SPS->FrameCroppingFlag                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->SPS->VUIPresent                                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseNALSequenceParameterSetExtended(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_extension_rbsp?
        if (Dec == NULL) {
            ...
            if (Dec->SPS->AuxiliaryFormatID != 0) {
                ...
                Dec->SPS->AlphaIncrFlag                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SPS->AlphaOpaqueValue                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->AuxiliaryBitDepth + 9);
                Dec->SPS->AlphaTransparentValue                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->AuxiliaryBitDepth + 9);
            }
            Dec->SPS->AdditionalExtensionFlag                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            BitBuffer_Align(BitB, 1); // rbsp_trailing_bits
        }
    }
    ...
    void ParseNALSubsetSPS(DecodeAVC *Dec, BitBuffer *BitB) { // subset_seq_parameter_set_rbsp
        if (Dec == NULL) {
            ...
            if ((Dec->SPS->ProfileIDC == Scalable_Constrained_Baseline_Profile) || (Dec->SPS->ProfileIDC == Scalable_High_Intra_Profile)) { // Scalable Video mode
                ...
                Dec->SPS->MVCVUIParamsPresent                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->SPS->AdditionalExtension2                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SPS->AdditionalExtension2 == true) {
                while (more_rbsp_data()) {
                    Dec->SPS->AdditionalExtension2DataFlag                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
            }
            BitBuffer_Align(BitB, 1); // rbsp_trailing_bits
        }
    ...
    void ParseVideoUsabilityInformation(DecodeAVC *Dec, BitBuffer *BitB) { // Video Usability Information; ParseVUIParameters
        if (Dec == NULL) {
            ...
            Dec->VUI->AspectRatioInfoPresent                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->VUI->AspectRatioInfoPresent == true) {
                Dec->VUI->AspectRatioIDC                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                if (Dec->VUI->AspectRatioIDC == 255) { // Extended_SAR = 255
                    Dec->VUI->SARWidth                                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->VUI->SAWHeight                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                }
            }
            Dec->VUI->OverscanInfoPresent                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->VUI->OverscanInfoPresent == true) {
                Dec->VUI->DisplayInOverscan                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->VUI->VideoSignalTypePresent                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->VUI->VideoSignalTypePresent == true) {
                Dec->VUI->VideoType                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->VUI->FullRange                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->VUI->ColorDescriptionPresent                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->ColorDescriptionPresent == true) {
                    Dec->VUI->ColorPrimaries                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                    Dec->VUI->TransferProperties                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                    Dec->VUI->MatrixCoefficients                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                }
            }
            Dec->VUI->ChromaLocationPresent                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->VUI->TimingInfoPresent[0]                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->VUI->TimingInfoPresent[0] == true) {
                Dec->VUI->UnitsPerTick[0]                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                Dec->VUI->TimeScale[0]                                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                Dec->VUI->FixedFrameRateFlag[0]                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->VUI->NALHrdParamsPresent[0]                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->VUI->NALHrdParamsPresent[0] == true) {
                ParseHypotheticalReferenceDecoder(Dec, BitB);
            }
            Dec->VUI->VCLHrdParamsPresent[0]                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            if ((Dec->VUI->NALHrdParamsPresent[0] || Dec->VUI->VCLHrdParamsPresent[0]) == true) {
                //Dec->StreamIsLowDelay                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->VUI->PicStructPresent[0]                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->VUI->BitStreamRestricted                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->VUI->BitStreamRestricted == true) {
                Dec->VUI->MotionVectorsOverPicBoundaries[0]                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseMVCDVUIParametersExtension(DecodeAVC *Dec, BitBuffer *BitB) { // mvcd_vui_parameters_extension
        if (Dec == NULL) {
            ...
            for (uint8_t MVCDOpPoint = 0; MVCDOpPoint < Dec->VUI->VUIMVCDNumOpPoints; MVCDOpPoint++) {
                Dec->VUI->VUIMVCDTemporalID[MVCDOpPoint]                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                ...
                Dec->VUI->VUIMVCDTimingInfoPresent[MVCDOpPoint]                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->VUIMVCDTimingInfoPresent[MVCDOpPoint] == true) {
                    Dec->VUI->VUIMVCDNumUnitsInTick[MVCDOpPoint]                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->VUI->VUIMVCDTimeScale[MVCDOpPoint]                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->VUI->VUIMVCDFixedFrameRateFlag[MVCDOpPoint]                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->VUI->VUIMVCDNalHRDParametersPresent[MVCDOpPoint]                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->VUIMVCDNalHRDParametersPresent[MVCDOpPoint] == true) {
                    hrd_parameters();
                }
                Dec->VUI->VUIMVCDVclHRDParametersPresent[MVCDOpPoint]                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if ((Dec->VUI->VUIMVCDNalHRDParametersPresent[MVCDOpPoint] || Dec->VUI->VUIMVCDVclHRDParametersPresent[MVCDOpPoint]) == true) {
                    Dec->VUI->VUIMVCDHRDLowDelayPresent[MVCDOpPoint]                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->VUI->VUIMVCDPicStructPresent[MVCDOpPoint]                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
    }
    ...
    void ParseMVCVUIParametersExtension(DecodeAVC *Dec, BitBuffer *BitB) { // mvc_vui_parameters_extension
        if (Dec == NULL) {
            ...
            for (uint16_t Operation = 0; Operation < Dec->VUI->MVCNumOpertionPoints; Operation++) {
                Dec->VUI->MVCTemporalID[0][Operation]                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                ...
                Dec->VUI->TimingInfoPresent[Operation]                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->TimingInfoPresent[Operation] == true) {
                    Dec->VUI->MVCUnitsInTick[Operation]                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->VUI->MVCTimeScale[Operation]                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->VUI->MVCFixedFrameRate[Operation]                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->VUI->MVCNALHRDParamsPresent[Operation]                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->MVCNALHRDParamsPresent[Operation] == true) {
                    ParseHypotheticalReferenceDecoder(Dec, BitB);
                }
                Dec->VUI->MVCVCLHRDParamsPresent[Operation]                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if ((Dec->VUI->MVCNALHRDParamsPresent[Operation] || Dec->VUI->MVCVCLHRDParamsPresent[Operation]) == true) {
                    Dec->VUI->MVCLowDelayFlag[Operation]                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->VUI->PicStructPresent[Operation]                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
    }
    ...
    void ParseSVCVUIExtension(DecodeAVC *Dec, BitBuffer *BitB) { // svc_vui_parameters_extension
        if (Dec == NULL) {
            ...
            for (uint8_t VUIExtEntry = 0; VUIExtEntry < Dec->VUI->VUIExtNumEntries; VUIExtEntry++) {
                Dec->VUI->VUIExtDependencyID[VUIExtEntry]                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->VUI->VUIExtQualityID[VUIExtEntry]                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                Dec->VUI->VUIExtTemporalID[VUIExtEntry]                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->VUI->VUIExtTimingInfoPresentFlag[VUIExtEntry]                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->VUIExtTimingInfoPresentFlag[VUIExtEntry] == true) {
                    Dec->VUI->VUIExtNumUnitsInTick[VUIExtEntry]                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->VUI->VUIExtTimeScale[VUIExtEntry]                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->VUI->VUIExtFixedFrameRateFlag[VUIExtEntry]                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->VUI->VUIExtNALHRDPresentFlag[VUIExtEntry]                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->VUI->VUIExtNALHRDPresentFlag[VUIExtEntry] == true) {
                    ParseHypotheticalReferenceDecoder(Dec, BitB);
                }
                Dec->VUI->VUIExtVCLHRDPresentFlag[VUIExtEntry]                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if ((Dec->VUI->VUIExtNALHRDPresentFlag[VUIExtEntry] == true) || (Dec->VUI->VUIExtVCLHRDPresentFlag[VUIExtEntry] == true)) {
                    Dec->VUI->VUIExtLowDelayHRDFlag[VUIExtEntry]                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->VUI->VUIExtPicStructPresentFlag[VUIExtEntry]                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
    }
    ...
    void ParseNALPictureParameterSet(DecodeAVC *Dec, BitBuffer *BitB) { // pic_parameter_set_rbsp
        if (Dec == NULL) {
            ...
            Dec->PPS->EntropyCodingMode                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // bit 9, 0
            Dec->PPS->BottomPicFieldOrderInSliceFlag                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // bit 8, 0
            ...
            if (Dec->PPS->SliceGroups > 0) {
                ...
                if (Dec->PPS->SliceGroupMapType == 0) {
                    ...
                    for (uint8_t SliceGroup = 0; SliceGroup <= Dec->PPS->SliceGroups; SliceGroup++) {
                        Dec->PPS->SliceGroupChangeDirection                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    ...
                    for (uint64_t MapUnit = 0; MapUnit <= Dec->PPS->PicSizeInMapUnits; MapUnit++) {
                        Dec->PPS->SliceGroupID[MapUnit]                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Ceili(log2(Dec->PPS->SliceGroups)));
                    }
                }
            }
            ...
            Dec->PPS->WeightedPrediction                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->PPS->WeightedBiPrediction                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->PPS->DeblockingFilterFlag                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->PPS->ConstrainedIntraFlag                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->PPS->RedundantPictureFlag                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (more_rbsp_data() == true) {
                Dec->PPS->TransformIs8x8                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->PPS->SeperateScalingMatrix                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->PPS->SeperateScalingMatrix == true) {
                    for (uint8_t i = 0; i < 6 + ((Dec->SPS->ChromaFormatIDC != Chroma444) ? 2 : 6) * Dec->PPS->TransformIs8x8; i++) {
                        Dec->PPS->PicScalingList[i]                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseNALSVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit_header_svc_extension
        if (Dec == NULL) {
            ...
            Dec->NAL->IDRFlag                                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->PriorityID[0]                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
            Dec->NAL->NoInterLayerPredictionFlag                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->DependencyID[0]                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            Dec->NAL->QualityID[0][0]                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->NAL->TemporalID[0]                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            Dec->NAL->UseReferenceBasePictureFlag                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->IsDisposable[0]                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->OutputFlag                                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseNALSequenceParameterSetSVC(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_svc_extension
        if (Dec == NULL) {
            ...
            Dec->SVC->InterLayerDeblockingFilterPresent                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SVC->ExtendedSpatialScalabilityIDC                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
            if ((Dec->SPS->ChromaFormatIDC == Chroma420) || (Dec->SPS->ChromaFormatIDC == Chroma422)) {
                Dec->SVC->ChromaPhaseXFlag                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            if (Dec->SPS->ChromaFormatIDC == Chroma420) {
                Dec->SVC->ChromaPhaseY                                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
            }
            if (Dec->SVC->ExtendedSpatialScalabilityIDC == 1) {
                if (Dec->SPS->ChromaFormatIDC != ChromaBW) {
                    Dec->SVC->SeqRefLayerChromaPhaseX                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SVC->SeqRefLayerChromaPhaseY                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                }
            ...
            Dec->SVC->SequenceCoeffLevelPresent                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SVC->SequenceCoeffLevelPresent == true) {
                Dec->SVC->AdaptiveCoeffsPresent                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->SVC->SliceHeaderRestricted                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
    }
    ...
    void ParseNALPrefixUnitSVC(DecodeAVC *Dec, BitBuffer *BitB) { // prefix_nal_unit_svc
        if (Dec == NULL) {
            ...
            if (Dec->NAL->NALRefIDC != 0) {
                Dec->Slice->StoreRefBasePicFlag                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                Dec->NAL->AdditionalPrefixNALExtensionFlag                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->NAL->AdditionalPrefixNALExtensionFlag == true) {
                    while (more_rbsp_data()) {
                        Dec->NAL->AdditionalPrefixNALExtensionDataFlag                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                }
                BitBuffer_Align(BitB, 1); // rbsp_trailing_bits()
    ...
    void ParseNALMVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit_header_mvc_extension
        if (Dec == NULL) {
            ...
            Dec->NAL->NonIDRFlag                                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->PriorityID[0]                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
            Dec->SPS->ViewID[0][0]                                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 10);
            Dec->NAL->TemporalID[0]                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            Dec->NAL->IsAnchorPicture                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->InterViewFlag                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            BitBuffer_Seek(BitB, 1);
        }
    }
    ...
    void ParseSPSMVCDExtension(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_mvcd_extension
        if (Dec == NULL) {
            ...
            for (uint16_t View = 0; View < Dec->SPS->ViewCount; View++) {
                ...
                Dec->SPS->DepthViewPresent[View]                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SPS->DepthViewID[View]                                             = Dec->SPS->ViewID[View][0];
                Dec->DPS->NumDepthViews                                                += Dec->SPS->DepthViewPresent[View];
                Dec->SPS->TextureViewPresent[View]                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            ...
            Dec->SPS->MVCDVUIParametersPresent                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SPS->MVCDVUIParametersPresent == true) {
                ParseMVCDVUIParametersExtension(Dec, BitB); // mvcd_vui_parameters_extension();
            }
            Dec->SPS->MVCDTextureVUIParametersPresent                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseNALDepthParameterSet(DecodeAVC *Dec, BitBuffer *BitB) { // depth_parameter_set_rbsp
        if (Dec == NULL) {
            ...
            if (Dec->DPS->PredictionDirection == 0) {
                ...
                Dec->DPS->PredictedWeight0                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
            }
            ...
            Dec->DPS->VSPParamFlag                                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->DPS->AdditionalExtensionFlag                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSPS3DAVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // seq_parameter_set_3davc_extension
        if (Dec == NULL) {
            ...
            if (Dec->DPS->NumDepthViews > 0) {
                ...
                Dec->SPS->ReducedResolutionFlag                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                Dec->SPS->DepthFrameCroppingFlag                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                Dec->SPS->SlicePrediction                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->Slice->SeqViewSynthesisFlag                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->SPS->ALCSpsEnableFlag                                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SPS->EnableRLESkipFlag                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseNAL3DAVCExtension(DecodeAVC *Dec, BitBuffer *BitB) { // nal_unit_header_3davc_extension
        if (Dec == NULL) {
            ...
            Dec->NAL->ViewIndex                                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
            Dec->NAL->DepthFlag                                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->NonIDRFlag                                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->TemporalID[0]                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            Dec->NAL->IsAnchorPicture                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->NAL->InterViewFlag                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseHypotheticalReferenceDecoder(DecodeAVC *Dec, BitBuffer *BitB) { // hrd_parameters
        if (Dec == NULL) {
            ...
            Dec->HRD->BitRateScale                                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->HRD->CodedPictureBufferScale                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            for (uint8_t SchedSelIdx = 0; SchedSelIdx < Dec->HRD->NumCodedPictureBuffers; SchedSelIdx++) {
                ...
                Dec->HRD->IsConstantBitRate[SchedSelIdx]                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1) + 1; // FIXME: is +1 correct
            }
            Dec->HRD->InitialCPBDelayLength                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5) + 1;
            Dec->HRD->CBPDelay                                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5) + 1;
            Dec->HRD->DBPDelay                                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5) + 1;
            Dec->HRD->TimeOffsetSize                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5);
        }
    }
    ...
    void ParseNALSliceHeader(DecodeAVC *Dec, BitBuffer *BitB) { // slice_header
        if (Dec == NULL) {
            ...
            if (Dec->SPS->SeperateColorPlane == true) {
                Dec->Slice->ColorPlaneID                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
            }

            Dec->Slice->FrameNumber                                                     = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false); // FIXME: Should I use ReadBits?
            if (Dec->SPS->OnlyMacroBlocksInFrame == false) {
                Dec->Slice->SliceIsInterlaced                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->Slice->SliceIsInterlaced == true) {
                    Dec->Slice->SliceIsBottomField                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
            }
            ...
            if ((Dec->Slice->Type == SliceB1) || (Dec->Slice->Type == SliceB2)) {
                Dec->Slice->DirectSpatialMVPredictionFlag                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            if ((Dec->Slice->Type == SliceP1)  || (Dec->Slice->Type == SliceP2)  ||
                ...
                Dec->Slice->NumRefIDXActiveOverrideFlag                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            if (
                ...
                (Dec->Slice->Type == SliceSI1) || (Dec->Slice->Type == SliceSI2)) {
                    ...
                    if ((Dec->Slice->Type == SliceSP1) || (Dec->Slice->Type == SliceSP2)) {
                        Dec->Slice->DecodePMBAsSPSlice                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
            ...
            if (Dec->PPS->SliceGroups > 0 && (Dec->PPS->SliceGroupMapType >= 3 && Dec->PPS->SliceGroupMapType <= 5)) {
                ...
                Dec->Slice->SliceGroupChangeCycle                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Ceili(log2(Dec->PPS->PicSizeInMapUnits /  Dec->Slice->SliceGroupChangeRate)));
            }
        }
    }
    ...
    void ParseNALFillerData(DecodeAVC *Dec, BitBuffer *BitB) { // filler_data_rbsp
        if (Dec == NULL) {
            ...
            while (PeekBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8) == 0xFF) {
                BitBuffer_Seek(BitB, 8);
            }
        }
    }
    ...
    void ParseNALAccessUnitDelimiter(DecodeAVC *Dec, BitBuffer *BitB) { // access_unit_delimiter_rbsp
        if (Dec == NULL) {
            ...
            Dec->Slice->PictureType                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            BitBuffer_Align(BitB, 1);
        }
    }
    ...
    void ParseSEIBufferingPeriod(DecodeAVC *Dec, BitBuffer *BitB) { // buffering_period
        if (Dec == NULL) {
            ...
            if (Dec->SEI->NalHrdBpPresentFlag == true) {
                for (uint8_t SchedSelIdx = 0; SchedSelIdx <= Dec->HRD->NumCodedPictureBuffers; SchedSelIdx++) {
                    Dec->SEI->InitialCPBRemovalDelay[SchedSelIdx]                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->HRD->InitialCPBDelayLength);
                    Dec->SEI->InitialCPBRemovalDelayOffset[SchedSelIdx]                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->HRD->InitialCPBDelayLength);
                }
            }
    ...
    void ParseSEIPictureTiming(DecodeAVC *Dec, BitBuffer *BitB) { // pic_timing
        if (Dec == NULL) {
            ...
            if (Dec->SEI->CpbDpbDelaysPresentFlag == true) {
                Dec->SEI->CPBRemovalDelay                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->HRD->CBPDelay);
                Dec->SEI->DPBOutputDelay                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->HRD->DBPDelay);
            }
            if (Dec->VUI->PicStructPresent[0] == true) {
                Dec->SEI->PicStruct                                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                uint8_t NumClockTS = GetClockTS(Dec->SEI->PicStruct);
                for (uint8_t Time = 0; Time < NumClockTS; Time++) {
                    Dec->SEI->ClockTimestampFlag[Time]                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    if (Dec->SEI->ClockTimestampFlag[Time] == true) {
                        Dec->SEI->CTType                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                        Dec->SEI->NuitFieldBasedFlag                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        Dec->SEI->CountingType                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5);
                        Dec->SEI->FullTimestampFlag                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        Dec->SEI->CountDroppedFlag                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        Dec->SEI->NFrames                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                        if (Dec->SEI->FullTimestampFlag == true) {
                            Dec->SEI->Seconds                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                            Dec->SEI->Minutes                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                            Dec->SEI->Hours                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5);
                        } else {
                            Dec->SEI->SecondsFlag                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            if (Dec->SEI->SecondsFlag == true) {
                                Dec->SEI->Seconds                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                                Dec->SEI->MinutesFlag                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                                if (Dec->SEI->MinutesFlag == true) {
                                    Dec->SEI->Minutes                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                                    Dec->SEI->HoursFlag                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            ...
                            if (Dec->HRD->TimeOffsetSize > 0) {
                                Dec->SEI->TimeOffset                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->HRD->TimeOffsetSize);
                            }
                        }
                    }
                }
    ...
    void ParseSEIPanScan(DecodeAVC *Dec, BitBuffer *BitB) { // pan_scan_rect
        if (Dec == NULL) {
            ...
            Dec->SEI->DisablePanScanFlag                                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSEIRegisteredUserData(DecodeAVC *Dec, BitBuffer *BitB) { // user_data_registered_itu_t_t35
        if (Dec == NULL) {
            ...
            Dec->SEI->CountryCode                                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
    ...
    void ParseSEIRecoveryPoint(DecodeAVC *Dec, BitBuffer *BitB) { // recovery_point
        if (Dec == NULL) {
            ...
            Dec->SEI->ExactMatchFlag                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->BrokenLinkFlag                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->ChangingSliceGroupIDC                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
        }
    }
    ...
    void ParseSEIRepetitiveReferencePicture(DecodeAVC *Dec, BitBuffer *BitB) { // dec_ref_pic_marking_repetition
        if (Dec == NULL) {
            ...
            Dec->SEI->OriginalIDRFlag                                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->OriginalFrameNum                                                  = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false);
            if (Dec->SPS->OnlyMacroBlocksInFrame == false) {
                Dec->SEI->OriginalFieldPicFlag                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->OriginalFieldPicFlag == true) {
                    Dec->SEI->OriginalBottomFieldFlag                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
            }
    ...
    void ParseSEISparePicture(DecodeAVC *Dec, BitBuffer *BitB) { // spare_pic
        if (Dec == NULL) {
            ...
            Dec->SEI->SpareFieldFlag                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->SpareFieldFlag == true) {
                Dec->SEI->TargetBottomFieldFlag                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            ...
            for (uint8_t SparePic = 0; SparePic < Dec->SEI->NumSparePics; SparePic++) {
                ...
                if (Dec->SEI->SpareAreaIDC[SparePic] == 1) {
                    for (uint8_t MapUnit = 0; MapUnit < Dec->PPS->PicSizeInMapUnits; MapUnit++) {
                        Dec->SEI->SpareUnitFlag[SparePic][MapUnit]                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
    ...
    void ParseSEISceneInfo(DecodeAVC *Dec, BitBuffer *BitB) { // scene_info
        if (Dec == NULL) {
            ...
            Dec->SEI->SceneInfoPresentFlag                                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSEISubSequenceInfo(DecodeAVC *Dec, BitBuffer *BitB) { // sub_seq_info
        if (Dec == NULL) {
            ...
            Dec->SEI->FirstRefPicFlag                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->LeadingNonRefPicFlag                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->LastPicFlag                                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->SubSeqFrameNumFlag                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSEISubSequenceLayerProperties(DecodeAVC *Dec, BitBuffer *BitB) { // sub_seq_layer_characteristics
        if (Dec == NULL) {
            ...
            for (uint8_t Layer = 0; Layer < Dec->SEI->NumSubSeqLayers; Layer++) {
                Dec->SEI->AccurateStatisticsFlag                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->AverageBitRate                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                Dec->SEI->AverageFrameRate                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            }
        }
    }
    ...
    void ParseSEISubSequenceProperties(DecodeAVC *Dec, BitBuffer *BitB) { // sub_seq_characteristics
        if (Dec == NULL) {
            ...
            Dec->SEI->SubSequenceLayerNum                                                = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false);
            Dec->SEI->SubSequenceID                                                      = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false);
            Dec->SEI->DurationFlag                                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->DurationFlag == true) {
                Dec->SEI->SubSeqDuration                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
            }
            Dec->SEI->AverageRateFlag                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            for (uint16_t SubSequence = 0; SubSequence < Dec->SEI->NumReferencedSubSeqs; SubSequence++) {
                ...
                Dec->SEI->RefSubSeqDirection                                             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
    }
        ...
        if (Dec == NULL) {
            ...
            if (Dec->SEI->NumSliceGroupsInSet > 1) {
                for (uint16_t SliceGroup = 0; SliceGroup < Dec->SEI->NumSliceGroupsInSet; SliceGroup++) {
                    Dec->PPS->SliceGroupID[SliceGroup]                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Ceili(log2(Dec->PPS->SliceGroups)));
                }
            }
            Dec->SEI->ExactSampleValueMatchFlag[0]                                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->PanScanRectFlag                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSEIFilmGrainCharacteristics(DecodeAVC *Dec, BitBuffer *BitB) { // film_grain_characteristics
        if (Dec == NULL) {
            ...
            Dec->SEI->FilmGrainCharactisticsCancelFlag                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->FilmGrainCharactisticsCancelFlag == false) {
                Dec->SEI->FilmGrainModelID                                               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                Dec->SEI->SeperateColorDescriptionFlag                                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->SeperateColorDescriptionFlag == true) {
                    Dec->SEI->FilmGrainBitDepthLuma                                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3) + 8;
                    Dec->SEI->FilmGrainBitDepthChroma                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3) + 8;
                    Dec->SEI->FilmGrainFullRangeFlag                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->FilmGrainColorPrimaries                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                    Dec->SEI->FilmGrainTransferCharacteristics                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                    Dec->SEI->FilmGrainMatrixCoefficents                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                }
                Dec->SEI->BlendingModeID                                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                Dec->SEI->Scalefactor                                                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                for (uint8_t Channel = 0; Channel < 3; Channel++) {
                    Dec->SEI->CompModelPresent[Channel]                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                for (uint8_t Channel = 0; Channel < 3; Channel++) {
                    if (Dec->SEI->CompModelPresent[Channel] == true) {
                        Dec->SEI->NumIntensityIntervals[Channel]                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8) + 1;
                        Dec->SEI->NumModelValues[Channel]                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3) + 1;
                        for (uint16_t Intensity = 0; Intensity < Dec->SEI->NumIntensityIntervals[Channel]; Intensity++) {
                            Dec->SEI->IntensityIntervalLowerBound[Channel][Intensity]    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                            Dec->SEI->IntensityIntervalUpperBound[Channel][Intensity]    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
    ...
    void ParseSEIDeblockingFilterDisplayPreference(DecodeAVC *Dec, BitBuffer *BitB) { // deblocking_filter_display_preference
        if (Dec == NULL) {
            ...
            Dec->SEI->DeblockingDisplayPreferenceCancelFlag           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->DeblockingDisplayPreferenceCancelFlag == false) {
                Dec->SEI->DisplayBeforeDeblockingFlag                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->DecodeFrameBufferingConstraintFlag          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSEIStereoVideoInfo(DecodeAVC *Dec, BitBuffer *BitB) { // stereo_video_info
        if (Dec == NULL) {
            ...
            Dec->SEI->FieldViewsFlag               = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->FieldViewsFlag == true) {
                Dec->SEI->TopFieldIsLeftViewFlag       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            } else {
                Dec->SEI->CurrentFrameIsLeftViewFlag   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->NextFrameIsDependentViewFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            Dec->SEI->LeftViewSelfContainedFlag    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->RightViewSelfContainedFlag   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
    }
    ...
    void ParseSEIPostFilterHint(DecodeAVC *Dec, BitBuffer *BitB) { // post_filter_hint
        if (Dec == NULL) {
            ...
            Dec->SEI->FilterHintType      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
    ...
    void ParseSEIToneMappingInfo(DecodeAVC *Dec, BitBuffer *BitB) { // tone_mapping_info
        if (Dec == NULL) {
            ...
            Dec->SEI->ToneMapCancelFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->ToneMapCancelFlag == false) {
                ...
                Dec->SEI->CodedDataBitDepth       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                Dec->SEI->TargetBitDepth   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                ...
                if (Dec->SEI->ToneMapModelID == 0) {
                    Dec->SEI->ToneMinValue = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->SEI->ToneMaxValue = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                } else if (Dec->SEI->ToneMapModelID == 1) {
                    Dec->SEI->SigmoidMidpoint = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->SEI->SigmoidWidth    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                } else if (Dec->SEI->ToneMapModelID == 2) {
                    for (uint16_t I = 0; I < (1 << Dec->SEI->TargetBitDepth); I++) {
                        Dec->SEI->StartOfCodedInterval[I] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, ((Dec->SEI->CodedDataBitDepth + 7) >> 3) << 3);
                    }
                } else if (Dec->SEI->ToneMapModelID == 3) {
                    Dec->SEI->NumPivots = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    for (uint16_t I = 0; I < Dec->SEI->NumPivots; I++) {
                        Dec->SEI->CodedPivotValue[I] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, ((Dec->SEI->CodedDataBitDepth + 7) >> 3) << 3);
                        Dec->SEI->TargetPivotValue[I] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, ((Dec->SEI->TargetBitDepth + 7) >> 3) << 3);
                    }
                } else if (Dec->SEI->ToneMapModelID == 4) {
                    Dec->SEI->CameraISOSpeedIDC = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                    if (Dec->SEI->CameraISOSpeedIDC == ExtendedISO) {
                        Dec->SEI->CameraISOSpeed = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    }
                    Dec->SEI->ExposureIndexIDC = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                    if (Dec->SEI->ExposureIndexIDC == ExtendedISO) {
                        Dec->SEI->ExposureIndexValue = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    }
                    Dec->SEI->ExposureCompensationSignFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->ExposureCompensationNumerator = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->ExposureCompensationDenominatorIDC = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->RefScreenLuminanceWhite = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->SEI->ExtendedRangeWhiteLevel = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->SEI->NominalBlackLumaCode    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->NominalWhiteLumaCode    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->ExtendedWhiteLumaCode   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                }
            }
        }
    }
    ...
    void ParseSEIScalabilityInfo(DecodeAVC *Dec, BitBuffer *BitB) { // scalability_info
        if (Dec == NULL) {
            ...
            Dec->SEI->TemporalIDNestingFlag                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->PriorityLayerInfoPresent                   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->PriorityIDSettingFlag                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            for (uint8_t Layer = 0; Layer < Dec->SEI->NumLayers; Layer++) {
                ...
                Dec->NAL->PriorityID[Layer]                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                Dec->NAL->IsDisposable[Layer]                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->NAL->DependencyID[Layer]                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->NAL->QualityID[0][Layer]                    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4); // FIXME; is this correct?
                Dec->NAL->TemporalID[Layer]                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->SEI->SubPicLayerFlag[Layer]                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->SubRegionLayerFlag[Layer]              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->IROIDivisionFlag[Layer]                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->ProfileLevelInfoPresentFlag[Layer]     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->BitRateInfoPresent[Layer]              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->FrameRateInfoPresent[Layer]            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->FrameSizeInfoPresentFlag[Layer]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->LayerDependencyInfoPresent[Layer]      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->ParameterSetsInfoPresent[Layer]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->BitstreamRestrictionInfoPresent[Layer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->ExactInterLayerPredFlag[Layer]         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if ((Dec->SEI->SubPicLayerFlag[Layer] == true) || (Dec->SEI->IROIDivisionFlag[Layer] == true)) {
                    Dec->SEI->ExactSampleValueMatchFlag[Layer]   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->SEI->LayerConversionFlag[Layer]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->LayerOutputFlag[Layer]                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->ProfileLevelInfoPresentFlag[Layer] == true) {
                    Dec->SEI->LayerProfileLevelID[Layer]         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
                }
                if (Dec->SEI->BitRateInfoPresent[Layer] == true) {
                    Dec->SEI->AvgBitRate[Layer]                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->MaxBitRate[Layer]                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->MaxBitRateRepresentation[Layer]    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->MaxBitRateWindow[Layer]            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                }
                if (Dec->SEI->FrameRateInfoPresent[Layer] == true) {
                    Dec->SEI->ConstantFrameRateIDC[Layer]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                    Dec->SEI->AvgFrameRate[Layer]                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                }
                ...
                if (Dec->SEI->SubRegionLayerFlag[Layer] == true) {
                    ...
                    Dec->SEI->DynamicRectFlag[Layer]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    if (Dec->SEI->DynamicRectFlag[Layer] == false) {
                        Dec->SEI->HorizontalOffset[Layer]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                        Dec->SEI->VerticalOffset[Layer]          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                        Dec->SEI->RegionWidth[Layer]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                        Dec->SEI->RegionHeight[Layer]            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    }
                }
                ...
                if (Dec->SEI->IROIDivisionFlag[Layer] == true) {
                    Dec->SEI->IROIGridFlag[Layer]                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if (Dec->SEI->LayerDependencyInfoPresent[Layer] == true) {
                    ...
                    if (Dec->SEI->LayerConversionFlag[Layer] == true) {
                        ...
                        for (uint8_t J = 0; J < 2; J++) {
                            Dec->SEI->RewritingInfoFlag[Layer][J]       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            if (Dec->SEI->RewritingInfoFlag[Layer][J] == true) {
                                Dec->SEI->RewritingProfileLevelIDC[Layer][J] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
                                Dec->SEI->RewritingAverageBitrate[Layer][J]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                                Dec->SEI->RewritingMaxBitrate[Layer][J]      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                            }
                        }
                    }
                }
            ...
            if (Dec->SEI->PriorityLayerInfoPresent == true) {
                Dec->SEI->NumDependencyLayersForPriorityLayer = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1) + 1;
                for (uint8_t DependentLayer = 0; DependentLayer < Dec->SEI->NumDependencyLayersForPriorityLayer; DependentLayer++) {
                    Dec->SEI->PriorityDependencyID[DependentLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                    ...
                    for (uint8_t PriorityLayer = 0; PriorityLayer < Dec->SEI->NumPriorityLayers[DependentLayer]; PriorityLayer++) {
                        ...
                        Dec->SEI->PriorityLevelIDC[DependentLayer][PriorityLayer]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
                        Dec->SEI->PriorityLevelAvgBitrate[DependentLayer][PriorityLayer]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                        Dec->SEI->PriorityLevelMaxBitrate[DependentLayer][PriorityLayer]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    }
                }
            }
            if (Dec->SEI->PriorityIDSettingFlag == true) {
                Dec->SEI->PriorityIDURIIndex = 0;
                Dec->SEI->PriorityIDSettingURI = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                for (uint8_t StringByte = Dec->SEI->PriorityIDSettingURI; StringByte > 0; StringByte--) {
                    Dec->SEI->PriorityIDSettingURIString[StringByte] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
                }
            }
        }
    }
    ...
    void ParseSEINonRequiredLayerRep(DecodeAVC *Dec, BitBuffer *BitB) { // non_required_layer_rep
        if (Dec == NULL) {
            ...
            for (uint32_t InfoEntry = 0; InfoEntry < Dec->SEI->NumInfoEntries; InfoEntry++) {
                Dec->SEI->EntryDependencyID[InfoEntry] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                ...
                for (uint8_t LayerRep = 0; LayerRep < Dec->SEI->NumNonRequiredLayerReps[InfoEntry]; LayerRep++) {
                    Dec->SEI->NonRequiredLayerRepDependencyID[InfoEntry][LayerRep] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                    Dec->SEI->NonRequiredLayerRepQualityID[InfoEntry][LayerRep] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                }
            }
        }
    }
    ...
    void ParseSEIPriorityLayerInfo(DecodeAVC *Dec, BitBuffer *BitB) { // priority_layer_info
        if (Dec == NULL) {
            ...
            Dec->SEI->PriorityDependencyID[0] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            Dec->SEI->NumPriorityIDs          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            for (uint8_t PriorityID = 0; PriorityID < Dec->SEI->NumPriorityIDs; PriorityID++) {
                Dec->SEI->AltPriorityIDs[PriorityID] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
            }
        }
    }
    ...
    void ParseSEIScalableNesting(DecodeAVC *Dec, BitBuffer *BitB) { // scalable_nesting
        if (Dec == NULL) {
            ...
            Dec->SEI->AllLayerRepresentationsInAUFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->AllLayerRepresentationsInAUFlag == false) {
                ...
                for (uint8_t LayerRepresentation = 0; LayerRepresentation < Dec->SEI->NumLayerRepresentations; LayerRepresentation++) {
                    Dec->SEI->SEIDependencyID[LayerRepresentation] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                    Dec->SEI->SEIQualityID[LayerRepresentation]    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                }
                Dec->SEI->SEITemporalID[0] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            }
            BitBuffer_Align(BitB, 1);
    ...
    void ParseSEIBaseLayerTemporalHRD(DecodeAVC *Dec, BitBuffer *BitB) { // base_layer_temporal_hrd
        if (Dec == NULL) {
            ...
            for (uint8_t TemporalLayer = 0; TemporalLayer < Dec->SEI->NumTemporalLayersInBaseLayer; TemporalLayer++) {
                Dec->SEI->SEITemporalID[TemporalLayer]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->SEI->SEITimingInfoPresent[TemporalLayer]      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->SEITimingInfoPresent[TemporalLayer] == true) {
                    Dec->SEI->SEIUnitsInTick[TemporalLayer]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->SEI->SEITimeScale[TemporalLayer]          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
                    Dec->SEI->SEIFixedFrameRateFlag[TemporalLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->SEI->SEINALHRDParamsPresentFlag[TemporalLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->SEINALHRDParamsPresentFlag[TemporalLayer] == true) {
                    ParseHypotheticalReferenceDecoder(Dec, BitB);
                }
                Dec->SEI->SEIVCLHRDParamsPresentFlag[TemporalLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if ((Dec->SEI->SEINALHRDParamsPresentFlag[TemporalLayer] == true) || (Dec->SEI->SEIVCLHRDParamsPresentFlag[TemporalLayer] == true)) {
                    Dec->SEI->SEILowDelayHRDFlag[TemporalLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->SEI->SEIPicStructPresentFlag[TemporalLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
    }
    ...
    void ParseSEIQualityLayerIntegrityCheck(DecodeAVC *Dec, BitBuffer *BitB) { // quality_layer_integrity_check
        if (Dec == NULL) {
            ...
            for (uint8_t IntegrityCheck = 0; IntegrityCheck < Dec->SEI->NumInfoEntries; IntegrityCheck++) {
                Dec->SEI->EntryDependencyID[IntegrityCheck] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                Dec->SEI->SEIQualityLayerCRC[IntegrityCheck] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            }
        }
    }
    ...
    void ParseSEIRedundantPicProperty(DecodeAVC *Dec, BitBuffer *BitB) { // redundant_pic_property
        if (Dec == NULL) {
            ...
            for (uint8_t DependencyID = 0; DependencyID < Dec->SEI->NumDependencyIDs; DependencyID++) {
                Dec->NAL->DependencyID[DependencyID]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                ...
                for (uint8_t QualityID = 0; QualityID < Dec->SEI->NumQualityIDs[DependencyID]; QualityID++) {
                    Dec->NAL->QualityID[DependencyID][QualityID] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                    ...
                    for (uint8_t RedundantPic = 0; RedundantPic < Dec->SEI->NumRedundantPics[DependencyID][QualityID]; RedundantPic++) {
                        ...
                        Dec->SEI->RedundantPicsMatch[DependencyID][QualityID][RedundantPic] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        if (Dec->SEI->RedundantPicsMatch[DependencyID][QualityID][RedundantPic] == false) {
                            Dec->SEI->MBTypeMatchFlag[DependencyID][QualityID][RedundantPic] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            Dec->SEI->MotionMatchFlag[DependencyID][QualityID][RedundantPic] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            Dec->SEI->ResidualMatchFlag[DependencyID][QualityID][RedundantPic] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            Dec->SEI->IntraSamplesMatchFlag[DependencyID][QualityID][RedundantPic] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        }
                    }
                }
            }
    ...
    void ParseSEITemporalDependencyRepresentationIndex(DecodeAVC *Dec, BitBuffer *BitB) { // tl0_dep_rep_index
        if (Dec == NULL) {
            ...
            Dec->SEI->TemporalDependencyRepresentationIndexLevel0 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
            Dec->SEI->EffectiveIDRPicID                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
        }
    }
    ...
    void ParseSEIMVCScalableNesting(DecodeAVC *Dec, BitBuffer *BitB) { // mvc_scalable_nesting
        if (Dec == NULL) {
            ...
            Dec->SEI->OperationPointFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->OperationPointFlag == false) {
                Dec->SEI->AllViewComponentsInAUFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->AllViewComponentsInAUFlag == false) {
                    ...
                    for (uint8_t ViewComponent = 0; ViewComponent < Dec->SEI->NumViewComponents; ViewComponent++) {
                        Dec->SEI->SEIViewID[ViewComponent] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 10);
                    }
                }
            } else {
                ...
                for (uint8_t ViewComponent = 0; ViewComponent < Dec->SEI->NumViewComponentsOp; ViewComponent++) {
                    Dec->SEI->SEIOpViewID[ViewComponent] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 10);
                }
                Dec->SEI->SEIOpTemporalID = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            }
            BitBuffer_Align(BitB, 1);
            ParseSEIMessage(Dec, BitB); // sei_message();
        }
    ...
    void ParseSEIViewScalabilityInfo(DecodeAVC *Dec, BitBuffer *BitB) { // view_scalability_info FIXME: FINISH THIS FUNCTION!!!
        if (Dec == NULL) {
            ...
            for (uint8_t OperationPoint = 0; OperationPoint < Dec->SEI->NumOperationPoints; OperationPoint++) {
                ...
                Dec->NAL->PriorityID[OperationPoint]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5);
                Dec->NAL->TemporalID[OperationPoint]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                ...
                Dec->SEI->ProfileLevelInfoPresentFlag[0]         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->BitRateInfoPresent[0]                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->FrameRateInfoPresent[0]                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->NumTargetOutputViews[OperationPoint] == false) {
                    Dec->SEI->ViewDependencyInfoPresent[OperationPoint] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                Dec->SEI->ParameterSetsInfoPresent[OperationPoint]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->BitstreamRestrictionInfoPresent[OperationPoint] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->SEI->ProfileLevelInfoPresentFlag[0] == true) {
                    Dec->SEI->OpProfileLevelIDC[OperationPoint]           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
                }
                if (Dec->SEI->BitstreamRestrictionInfoPresent[OperationPoint] == true) {
                    Dec->SEI->AvgBitRate[OperationPoint]                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->MaxBitRate[OperationPoint]                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    Dec->SEI->MaxBitRateWindow[OperationPoint]            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                }
                if (Dec->SEI->FrameRateInfoPresent[0] == true) {
                    Dec->SEI->ConstantFrameRateIDC[OperationPoint]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                    Dec->SEI->AvgFrameRate[OperationPoint]                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                }
                ...
                if (Dec->SEI->BitstreamRestrictionInfoPresent[OperationPoint] == true) {
                    Dec->VUI->MotionVectorsOverPicBoundaries[OperationPoint]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                if (Dec->SEI->LayerConversionFlag[OperationPoint] == true) {
                    ...
                    for (uint8_t J = 0; J < 2; J++) {
                        Dec->SEI->RewritingInfoFlag[OperationPoint][J] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        for (uint8_t RewriteInfo = 0; RewriteInfo < Dec->SEI->RewritingInfoFlag[OperationPoint][J]; RewriteInfo++) {
                            Dec->SEI->RewritingProfileLevelIDC[OperationPoint][J] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
                            Dec->SEI->RewritingAverageBitrate[OperationPoint][J]  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                            Dec->SEI->RewritingMaxBitrate[OperationPoint][J]      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                        }
                    }
                }
            }
            if (Dec->SEI->PriorityLayerInfoPresent == true) {
                ...
                for (uint8_t DependencyLayer = 0; DependencyLayer < Dec->SEI->NumDependencyLayersForPriorityLayer; DependencyLayer++) {
                    Dec->SEI->PriorityDependencyID[DependencyLayer]                       = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                    ...
                    for (uint8_t PriorityLayer = 0; PriorityLayer < Dec->SEI->NumPriorityLayers[DependencyLayer]; PriorityLayer++) {
                        ...
                        Dec->SEI->PriorityLevelIDC[DependencyLayer][PriorityLayer]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 24);
                        Dec->SEI->PriorityLevelAvgBitrate[DependencyLayer][PriorityLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                        Dec->SEI->PriorityLevelMaxBitrate[DependencyLayer][PriorityLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                    }
                }
            }
            if (Dec->SEI->PriorityIDSettingFlag == true) {
    ...
    void ParseSEIMVCAcquisitionInfo(DecodeAVC *Dec, BitBuffer *BitB) { // multiview_acquisition_info
        if (Dec == NULL) {
            ...
            Dec->SPS->ViewCount            = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false) + 1;
            Dec->SEI->IntrinsicParamFlag   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->ExtrinsicParamFlag   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->IntrinsicParamFlag == true) {
                Dec->SEI->IntrinsicParamsEqual = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                ...
                for (uint8_t ParamSet = 0; ParamSet < Dec->SEI->NumParamSets; ParamSet++) { // FIXME: ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 0)
                    Dec->SEI->SignFocalLength[0][ParamSet]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->ExponentFocalLength[0][ParamSet]    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                    if (Dec->SEI->ExponentFocalLength[0][ParamSet] == 0) {
                        Dec->SEI->MantissaFocalLength[0][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->PrecFocalLength - 30));
                    } else {
                        Dec->SEI->MantissaFocalLength[0][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->ExponentFocalLength[0][ParamSet] + Dec->SEI->PrecFocalLength - 31));
                    }
                    Dec->SEI->SignFocalLength[1][ParamSet]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->ExponentFocalLength[1][ParamSet]    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                    if (Dec->SEI->ExponentFocalLength[1][ParamSet] == 0) {
                        Dec->SEI->MantissaFocalLength[1][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->PrecFocalLength - 30));
                    } else {
                        Dec->SEI->MantissaFocalLength[1][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->ExponentFocalLength[1][ParamSet] + Dec->SEI->PrecFocalLength - 31));
                    }
                    Dec->SEI->SignPrincipalPoint[0][ParamSet]     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->ExponentPrincipalPoint[0][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                    if (Dec->SEI->ExponentFocalLength[1][ParamSet] == 0) {
                        Dec->SEI->MantissaPrincipalPoint[0][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->PrecPrincipalPoint - 30));
                    } else {
                        Dec->SEI->MantissaPrincipalPoint[0][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->ExponentPrincipalPoint[0][ParamSet] + Dec->SEI->PrecPrincipalPoint - 31));
                    }
                    Dec->SEI->SignPrincipalPoint[1][ParamSet]     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->ExponentPrincipalPoint[1][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                    if (Dec->SEI->ExponentFocalLength[1][ParamSet] == 0) {
                        Dec->SEI->MantissaPrincipalPoint[1][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->PrecPrincipalPoint - 30));
                    } else {
                        Dec->SEI->MantissaPrincipalPoint[1][ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->ExponentPrincipalPoint[1][ParamSet] + Dec->SEI->PrecPrincipalPoint - 31));
                    }
                    Dec->SEI->SignSkewFactor[ParamSet]            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->SEI->ExponentSkewFactor[ParamSet]        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                    if (Dec->SEI->ExponentSkewFactor[ParamSet] == 0) {
                        Dec->SEI->MantissaSkewFactor[ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->PrecSkewFactor - 30));
                    } else {
                        Dec->SEI->MantissaSkewFactor[ParamSet] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Max(0, Dec->SEI->ExponentSkewFactor[ParamSet] + Dec->SEI->PrecSkewFactor - 31));
                    }
                }
            }
                ...
                for (uint8_t View = 0; View < Dec->SPS->ViewCount; View++) {
                    for (uint8_t Row = 1; Row <= 3; Row++) {
                        for (uint8_t Column = 1; Column <= 3; Column++) {
                            Dec->SEI->SignR[View][Row][Column]     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            Dec->SEI->ExponentR[View][Row][Column] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                            Dec->SEI->MantissaR[View][Row][Column] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 0);
                        }
                        Dec->SEI->SignT[View][Row] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        Dec->SEI->ExponentT[View][Row] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                        Dec->SEI->MantissaT[View][Row] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 0);
                    }
                }
            }
        }
    ...
    void ParseSEIViewDependencyChange(DecodeAVC *Dec, BitBuffer *BitB) { // view_dependency_change
        if (Dec == NULL) {
            ...
            Dec->SPS->SeqParamSetID       = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false);
            Dec->SEI->AnchorUpdateFlag    = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->NonAnchorUpdateFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->AnchorUpdateFlag == true) {
                for (uint8_t View = 0; View < Dec->SPS->ViewCount; View++) {
                    for (uint8_t AnchorRef = 0; AnchorRef < Dec->SPS->AnchorRefsCount[0][View]; AnchorRef++) {
                        Dec->SEI->AnchorRefFlag[0][View][AnchorRef] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                    for (uint8_t AnchorRef = 0; AnchorRef < Dec->SPS->AnchorRefsCount[1][View]; AnchorRef++) {
                        Dec->SEI->AnchorRefFlag[1][View][AnchorRef] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                }
            }
            if (Dec->SEI->NonAnchorUpdateFlag == true) {
                for (uint8_t View = 0; View < Dec->SPS->ViewCount; View++) {
                    for (uint8_t NonAnchorRef = 0; NonAnchorRef < Dec->SPS->NonAnchorRefCount[0][View][NonAnchorRef]; NonAnchorRef++) {
                        Dec->SEI->NonAnchorRefFlag[0][View][NonAnchorRef] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                    for (uint8_t NonAnchorRef = 0; NonAnchorRef < Dec->SPS->NonAnchorRefCount[1][View][NonAnchorRef]; NonAnchorRef++) {
                        Dec->SEI->NonAnchorRefFlag[1][View][NonAnchorRef] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                }
            }
        }
    ...
    void ParseSEIBaseViewTemporalHRD(DecodeAVC *Dec, BitBuffer *BitB) { // base_view_temporal_hrd
        if (Dec == NULL) {
            ...
            for (uint8_t TemporalLayer = 0; TemporalLayer < Dec->SEI->NumTemporalLayersInBaseView; TemporalLayer++) {
                Dec->SEI->SEIMVCTemporalID[TemporalLayer] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
            }
        }
    }
    ...
    void ParseSEIFramePackingArrangement(DecodeAVC *Dec, BitBuffer *BitB) { // frame_packing_arrangement
        if (Dec == NULL) {
            ...
            Dec->SEI->FramePackingArrangementCancelFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->FramePackingArrangementCancelFlag == false) {
                Dec->SEI->FramePackingArrangementType = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 7);
                Dec->SEI->QuincunxSamplingFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->ContentIntrepretationType = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 6);
                Dec->SEI->SpatialFlippingFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->FlippedFlagFrame0 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->FieldViewsFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->TheCurrentFrameIsFrame0 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->Frame0IsSelfContainedFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->Frame1IsSelfContainedFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if ((Dec->SEI->QuincunxSamplingFlag == false) && (Dec->SEI->FramePackingArrangementType != 5)) {
                    Dec->SEI->Frame0GridPositionX = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                    Dec->SEI->Frame0GridPositionY = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                    Dec->SEI->Frame1GridPositionX = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                    Dec->SEI->Frame1GridPositionY = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
                }
                Dec->SEI->FramePackingArrangementReservedByte = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8);
            ...
            Dec->SEI->FramePackingArrangementExtensionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
    }
    ...
    void ParseSEIMVCViewPosition(DecodeAVC *Dec, BitBuffer *BitB) { // multiview_view_position
        if (Dec == NULL) {
            ...
            Dec->SEI->MVCViewPositionExtensionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
    }
    ...
    void ParseSEIDisplayOrientation(DecodeAVC *Dec, BitBuffer *BitB) { // display_orientation
        if (Dec == NULL) {
            ...
            Dec->SEI->DisplayOrientationCancelFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->DisplayOrientationCancelFlag == false) {
                Dec->SEI->HorizontalFlip = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->VerticalFlip   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->CounterClockwiseRotation = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                Dec->SEI->DisplayOrientationRepetitionPeriod = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false);
                Dec->SEI->DisplayOrientationExtensionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
    }
    ...
    void ParseSEIDepthRepresentationInformation(DecodeAVC *Dec, BitBuffer *BitB) { // depth_representation_info
        if (Dec == NULL) {
            ...
            Dec->SEI->AllViewsEqual                                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->AllViewsEqual == true) {
                Dec->SPS->ViewCount                                     = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false) + 1;
            } else {
                Dec->SPS->ViewCount                                     = 1;
            }
            Dec->SEI->ZNearFlag                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->ZFarFlag                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if ((Dec->SEI->ZNearFlag == true) || (Dec->SEI->ZFarFlag == true)) {
                Dec->SEI->ZAxisEqualFlag                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->SEI->DMinFlag                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->DMaxFlag                                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
    ...
    void ParseSEI3DReferenceDisplayInfo(DecodeAVC *Dec, BitBuffer *BitB) { // three_dimensional_reference_displays_info
        if (Dec == NULL) {
            ...
            Dec->SEI->ReferenceViewingDistanceFlag                      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            for (uint8_t Display= 0; Display < Dec->SEI->NumReferenceDisplays; Display++) {
                ...
                Dec->SEI->ShiftPresentFlag[Display]                     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            Dec->SEI->ReferenceDisplays3DFlag                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
    }
    ...
    void ParseSEIDepthTiming(DecodeAVC *Dec, BitBuffer *BitB) { // depth_timing
        if (Dec == NULL) {
            ...
            Dec->SEI->PerViewDepthTimingFlag                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);

            if (Dec->SEI->PerViewDepthTimingFlag == true) {
                for (uint8_t View = 0; View < Dec->DPS->NumDepthViews; View++) {
                    Dec->SEI->OffsetLength[View]                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5) + 1;
                    Dec->SEI->DepthDisplayDelayOffsetFP[View]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SEI->OffsetLength[View]);
                    Dec->SEI->DepthDisplayDelayOffsetDP[View]             = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SEI->OffsetLength[View]);
                }
            }
        }
    }
    ...
    void ParseSEIDepthGridPosition(DecodeAVC *Dec, BitBuffer *BitB) { // depth_grid_position()
        if (Dec == NULL) {
            ...
            Dec->SEI->DepthGridPositionXFP                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 20);
            Dec->SEI->DepthGridPositionXDP                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->SEI->DepthGridPositionXSignFlag                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            Dec->SEI->DepthGridPositionYFP                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 20);
            Dec->SEI->DepthGridPositionYDP                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->SEI->DepthGridPositionYSignFlag                          = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
        }
    }
    ...
    void ParseSEIDepthSamplingInfo(DecodeAVC *Dec, BitBuffer *BitB) { // depth_sampling_info
        if (Dec == NULL) {
            ...
            Dec->SEI->DepthSampleWidthMul                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            Dec->SEI->DepthSampleWidthDP                                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->SEI->DepthSampleHeightMul                                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            Dec->SEI->DepthSampleHeightDP                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->SEI->PerViewDepthTimingFlag                              = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SEI->PerViewDepthTimingFlag == true) {
    ...
    void ParseSEIMeteringDisplayColorVolume(DecodeAVC *Dec, BitBuffer *BitB) { // mastering_display_color_volume
        if (Dec == NULL) {
            ...
            for (uint8_t Color = 0; Color < 3; Color++) {
                Dec->SEI->DisplayPrimariesX[Color]                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
                Dec->SEI->DisplayPrimariesY[Color]                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            }
            Dec->SEI->WhitePointX                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            Dec->SEI->WhitePointY                                         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 16);
            Dec->SEI->MaxDisplayMasteringLuminance                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
            Dec->SEI->MinDisplayMasteringLuminance                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 32);
        }
    }
        ...
        if (Dec == NULL) {
            ...
            if (Dec->SEI->OperationPointFlag == false) {
                ...
                if (Dec->SEI->AllViewComponentsInAUFlag == false) {
                    ...
                    for (uint8_t ViewComponent = 0; ViewComponent < Dec->SEI->NumViewComponents; ViewComponent++) {
                        Dec->SEI->SEIViewID[ViewComponent]                = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 10);
                        Dec->SEI->SEIViewApplicabilityFlag[ViewComponent] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                }
            } else {
                Dec->SEI->SEIOpTextureOnlyFlag                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                Dec->SEI->NumViewComponents                               = ReadExpGolomb(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, false) + 1;
                for (uint8_t ViewComponent = 0; ViewComponent < Dec->SEI->NumViewComponents; ViewComponent++) {
                    Dec->SEI->SEIOpViewID[ViewComponent]                  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 10);
                    if (Dec->SEI->SEIOpTextureOnlyFlag == false) {
                        Dec->SEI->SEIOpDepthFlag[ViewComponent]           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        Dec->SEI->SEIOpTextureFlag[ViewComponent]         = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                }
                Dec->SEI->SEIOpTemporalID                                 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
            BitBuffer_Align(BitB, 1);
            ParseSEIMessage(Dec, BitB); // sei_message();
        }
    ...
    void ParseSEIDepthRepresentationElement(BitBuffer *BitB, uint8_t OutSign, uint8_t OutExp, uint8_t OutMantissa, uint8_t OutManLen) { // depth_representation_sei_element
        if (BitB == NULL) {
            ...
            bool     DASignFlag                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // da_sign_flag
            uint8_t  DAExponent                                           = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 7); // da_exponent
            uint8_t  DAMatissaSize                                        = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 5) + 1;
            uint64_t DAMatissa                                            = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, DAMatissaSize);
        }
    }
    ...
    void ParseSEIMessage(DecodeAVC *Dec, BitBuffer *BitB) { // sei_message
        if (Dec == NULL) {
            ...
            Dec->SEI->SEIType += ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8); // last_payload_type_byte, 5

            while (PeekBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8) == 0xFF) {
                BitBuffer_Seek(BitB, 8);
                Dec->SEI->SEISize    += 255;
            }
            Dec->SEI->SEISize += ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 8); // last_payload_size_byte, 692, emulation prevention bytes not included, but these fields are.
            ...
            switch (Dec->SEI->SEIType) { // sei_payload
                ...
                case SEI_MasteringDisplayColorVolume:             // 137
                    ...
                    break;
                default:
                    Log(Severity_DEBUG, PlatformIO_FunctionName, "Unrecognized SEIType: %d", Dec->SEI->SEIType);
    ...
    void ParseAVC3DSlice(DecodeAVC *Dec, BitBuffer *BitB) { // slice_header_in_3davc_extension
        ...
        if ((Dec->NAL->AVC3DExtensionFlag == true) && (Dec->NAL->AVC3DExtensionFlag == true)) {
            Dec->Slice->PreSliceHeaderSrc      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
            if ((Dec->Slice->Type == SliceP1) || (Dec->Slice->Type == SliceP2) || (Dec->Slice->Type == SliceSP1) || (Dec->Slice->Type == SliceSP2) || (Dec->Slice->Type == SliceB1) || (Dec->Slice->Type == SliceB2)) {
                Dec->Slice->PreRefListSrc      = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
            ...
            if ((Dec->PPS->WeightedPrediction == true && ((Dec->Slice->Type == SliceP1) || (Dec->Slice->Type == SliceP2) || (Dec->Slice->Type == SliceSP1) || (Dec->Slice->Type == SliceSP2))) || ((Dec->PPS->WeightedBiPrediction == 1 && ((Dec->Slice->Type == SliceB1) || (Dec->Slice->Type == SliceB2))))) {
                ...
                Dec->Slice->PrePredictionWeightTableSrc = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
                if (Dec->Slice->PrePredictionWeightTableSrc == false) {
                    pred_weight_table(Dec, BitB);
                }
            }
            if (Dec->NAL->NALRefIDC != 0) {
                Dec->Slice->PreDecReferencePictureMarkingSrc = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2);
            ...
            Dec->Slice->FrameNumber     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 0); // FIXME: READ VARIABLE BITS frame_num
            if (Dec->SPS->OnlyMacroBlocksInFrame == false) {
                Dec->Slice->SliceIsInterlaced = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->Slice->SliceIsInterlaced == true) {
                    Dec->Slice->SliceIsBottomField  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            ...
            if (Dec->SPS->PicOrderCount == false) {
                Dec->Slice->PictureOrderCountLSB = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->MaxPicOrder);
                if ((Dec->PPS->BottomPicFieldOrderInSliceFlag == true) && (Dec->Slice->SliceIsInterlaced == false)) {
                    ...
                    (
                     ...
                     (Dec->Slice->Type == SliceSI2)
                        ...
                        if (Dec->NAL->DepthFlag == true) {
                            Dec->Slice->DepthWeightedPredictionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        } else if (Dec->NAL->AVC3DExtensionFlag == true) {
                            Dec->Slice->DMVPFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            if (Dec->Slice->SeqViewSynthesisFlag == true) {
                                Dec->Slice->SliceVSPFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                            }
                        }
    ...
    void ParseScalableSlice(DecodeAVC *Dec, BitBuffer *BitB) { // slice_header_in_scalable_extension
        ...
        if (Dec->NAL->QualityID == 0) {
            ...
            if ((Dec->NAL->NALUnitType == NAL_AuxiliarySliceExtension) && (((Dec->PPS->WeightedPrediction == true) &&((Dec->Slice->Type == SliceEP1)  || (Dec->Slice->Type == SliceEP2))) || ((Dec->PPS->WeightedBiPrediction == 1) && ((Dec->Slice->Type == SliceEB1) || (Dec->Slice->Type == SliceEB2))))) {
                if (Dec->NAL->NoInterLayerPredictionFlag == true) {
                    Dec->Slice->BasePredictionWeightTable = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
        ...
        if ((Dec->NAL->NoInterLayerPredictionFlag == true) && (Dec->NAL->QualityID == 0)) {
            ...
            Dec->Slice->ConstrainedIntraResamplingFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->SVC->ExtendedSpatialScalabilityIDC == 2) {
                ...
                if (Dec->SPS->ChromaArrayType > ChromaBW) {
                    ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->Slice->RefLayerChromaPhaseXFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    Dec->Slice->RefLayerChromaPhaseY     = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 2) - 1;
                }
        ...
        if (Dec->NAL->NoInterLayerPredictionFlag == true) {
            Dec->Slice->SkipSliceFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            if (Dec->Slice->SkipSliceFlag == true) {
                ...
                Dec->Slice->AdaptiveBaseModeFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                if (Dec->Slice->AdaptiveBaseModeFlag == false) {
                    Dec->Slice->DefaultBaseModeFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
                if (Dec->Slice->DefaultBaseModeFlag == false) {
                    Dec->Slice->AdaptiveMotionPredictionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    if (Dec->Slice->AdaptiveMotionPredictionFlag == false) {
                        Dec->Slice->DefaultMotionPredictionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                    }
                }
                if (Dec->Slice->AdaptiveResidualPredictionFlag == false) {
                    Dec->Slice->DefaultResidualPredictionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
            }
            if (Dec->SVC->AdaptiveCoeffsPresent == true) {
                Dec->Slice->TCoefficentPredictionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
        }
        if ((Dec->SVC->SliceHeaderRestricted == false) && (Dec->Slice->SkipSliceFlag == false)) {
            Dec->Slice->ScanIndexStart = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
            Dec->Slice->ScanIndexEnd   = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 4);
        }
    }
    ...
    void ParseScalableSliceData(DecodeAVC *Dec, BitBuffer *BitB) { // slice_data_in_scalable_extension
        ...
        if (moreDataFlag == true) {
            if (Dec->Slice->MbaffFrameFlag && ((Dec->Slice->CurrentMacroBlockAddress % 2) == 0 || (((Dec->Slice->CurrentMacroBlockAddress % 2) == 1) && (Dec->Slice->PreviousMacroBlockSkipped == true)))) {
                Dec->Slice->MacroBlockFieldDecodingFlag  = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
            }
    ...
    void ParseMacroBlockLayerInSVC(DecodeAVC *Dec, BitBuffer *BitB) { // macroblock_layer_in_scalable_extension
        if (Dec->PPS->EntropyCodingMode == ExpGolomb) {
            if (InCropWindow(Dec, Dec->Slice->CurrentMacroBlockAddress) && (Dec->Slice->AdaptiveBaseModeFlag == true)) {
                Dec->MacroBlock->BaseModeFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // base_mode_flag
            }
            ...
            if (Dec->MacroBlock->Type == I_PCM) {
                ...
                for (uint16_t Sample = 0; Sample < MacroBlockMaxPixels; Sample++) {
                    Dec->MacroBlock->PCMLumaSample[Sample] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->LumaBitDepthMinus8);
                }
                for (uint8_t Channel = 0; Channel < 2; Channel++) {
                    for (uint8_t Sample = 0; Sample < (2 * (Dec->SPS->MacroBlockWidthChroma * Dec->SPS->MacroBlockHeightChroma)); Sample++) {
                        Dec->MacroBlock->PCMChromaSample[Channel][Sample] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->ChromaBitDepthMinus8);
                    }
                }
            } else {
                if (Dec->MacroBlock->BaseModeFlag == false) {
                    ...
                    if ((Dec->MacroBlock->Type != I_NxN) && (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Intra_16x16) && NumMacroBlockPartitions(Dec->MacroBlock->Type) == 4) {
                        ...
                        if (Dec->PPS->TransformIs8x8 == true && Dec->MacroBlock->Type == I_NxN) {
                            Dec->MacroBlock->TransformSizeIs8x8 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        }
                ...
                if ((Dec->Slice->AdaptiveResidualPredictionFlag == true) && ((Dec->Slice->Type != SliceEI1) || (Dec->Slice->Type != SliceEI2)) && InCropWindow(Dec, Dec->Slice->CurrentMacroBlockAddress) && (Dec->MacroBlock->BaseModeFlag || (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Intra_16x16 && MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Intra_8x8 && MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Intra_4x4))) {
                    Dec->Slice->ResidualPredictionFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                }
            ...
            if (Dec->MacroBlock->Type == I_PCM) {
                ...
                if (Dec->Slice->ScanIndexEnd >= Dec->Slice->ScanIndexStart) {
                    if (Dec->MacroBlock->BaseModeFlag || MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Intra_16x16) {
                        ...
                        if (Dec->MacroBlock->BlockPatternLuma > 0 && Dec->PPS->TransformIs8x8 && (Dec->MacroBlock->BaseModeFlag || (Dec->MacroBlock->Type != I_NxN && Dec->MacroBlock->NoMBSmallerThan8x8Flag && (Dec->MacroBlock->Type != B_Direct_16x16 || Dec->SPS->Inference8x8)))) {
                            Dec->MacroBlock->TransformSizeIs8x8 = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1); // OR Arithmetic
                        }
                    }
                    if (Dec->MacroBlock->BlockPatternLuma > 0 || Dec->MacroBlock->BlockPatternChroma > 0 || MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) == Intra_16x16) {
            ...
            if ((MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) == Intra_4x4) || (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) == Intra_8x8) || (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) == Intra_16x16)) {
                if (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) == Intra_4x4) {
                    for (uint8_t luma4x4BlkIdx = 0; luma4x4BlkIdx < 16; luma4x4BlkIdx++) {
                        Dec->MacroBlock->PreviousIntra4x4PredictionFlag[luma4x4BlkIdx] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        if (!Dec->MacroBlock->PreviousIntra4x4PredictionFlag[luma4x4BlkIdx]) {
                            Dec->MacroBlock->RemIntra4x4PredictionMode[luma4x4BlkIdx] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                        }
                    }
                }
                if (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) == Intra_8x8) {
                    for (uint8_t luma8x8BlkIdx = 0; luma8x8BlkIdx < 4; luma8x8BlkIdx++) {
                        Dec->MacroBlock->PreviousIntra8x8PredictionFlag[luma8x8BlkIdx] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        if (!Dec->MacroBlock->PreviousIntra8x8PredictionFlag[luma8x8BlkIdx]) {
                            Dec->MacroBlock->RemIntra8x8PredictionMode[luma8x8BlkIdx] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                        }
                    }
                }
                if (Dec->SPS->ChromaArrayType != 0) {
                ...
                if (InCropWindow(Dec, Dec->Slice->CurrentMacroBlockAddress) && Dec->Slice->AdaptiveMotionPredictionFlag) {
                    for (uint32_t MacroBlockPiece = 0; MacroBlockPiece < NumMacroBlockPartitions(Dec->MacroBlock->Type); MacroBlockPiece++) {
                        if (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Pred_L1) {
                            Dec->MacroBlock->MotionPredictionFlagLevel0[MacroBlockPiece] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        }
                        if (MacroBlockPartitionPredictionMode(Dec, Dec->MacroBlock->Type, 0) != Pred_L0) {
                            Dec->MacroBlock->MotionPredictionFlagLevel1[MacroBlockPiece] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        }
                    }
                }
                for (uint32_t MacroBlockPiece = 0; MacroBlockPiece < NumMacroBlockPartitions(Dec->MacroBlock->Type); MacroBlockPiece++) {
        ...
        if (Dec->PPS->EntropyCodingMode == ExpGolomb) {
            ...
            if (Dec->MacroBlock->Type == I_PCM) { // I_PCM
                ...
                for (uint16_t i = 0; i < 256; i++) {
                    Dec->MacroBlock->PCMLumaSample[i] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->LumaBitDepthMinus8);
                    for (uint8_t j = 0; j < 2; j++) { // FIXME: 2 needs to be a variable of the number of channels
                        Dec->MacroBlock->PCMChromaSample[j][i] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, Dec->SPS->ChromaBitDepthMinus8);
                    }
                }
            } else { // NOT I_PCM
            ...
            if ((MacroBlockPredictionMode == Intra_4x4) || (MacroBlockPredictionMode == Intra_8x8) || (MacroBlockPredictionMode == Intra_16x16)) {
                if (MacroBlockPredictionMode == Intra_4x4) {
                    for (uint8_t Luma4x4Block = 0; Luma4x4Block < 16; Luma4x4Block++) { // luma4x4BlkIdx
                        Dec->MacroBlock->PreviousIntra4x4PredictionFlag[Luma4x4Block] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        if (Dec->MacroBlock->PreviousIntra4x4PredictionFlag[Luma4x4Block] == false) {
                            Dec->MacroBlock->RemIntra4x4PredictionMode[Luma4x4Block] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                        }
                    }
                }
                if (MacroBlockPredictionMode == Intra_8x8) {
                    for (uint8_t Luma8x8Block = 0; Luma8x8Block < 4; Luma8x8Block++) { // luma8x8BlkIdx
                        Dec->MacroBlock->PreviousIntra8x8PredictionFlag[Luma8x8Block] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
                        if (Dec->MacroBlock->PreviousIntra8x8PredictionFlag[Luma8x8Block] == false) {
                            Dec->MacroBlock->RemIntra8x8PredictionMode[Luma8x8Block] = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 3);
                        }
                    }
                }
                if ((Dec->SPS->ChromaArrayType == Chroma420) || (Dec->SPS->ChromaArrayType == Chroma422)) {
    ...
    void ParseReferenceBasePictureSyntax(DecodeAVC *Dec, BitBuffer *BitB) { // dec_ref_base_pic_marking
        Dec->SVC->AdaptiveMarkingModeFlag = ReadBits(ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, BitB, 1);
