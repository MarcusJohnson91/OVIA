...
extern "C" {
    ...
    void FLAC_Frame_Read(FLACOptions *Options, BitBuffer *BitB, Audio2DContainer *Audio) {
        ...
        Options->Frame->CodedSampleRate  = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4); // 9
        Options->Frame->ChannelLayout    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4); // 10
        Options->Frame->CodedBitDepth    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 3); // 8
        
        BitBuffer_Seek(BitB, 1); // Reserved2, should be 0
        
        uint8_t Frame_SampleIDSize    = BitBuffer_ReadUnary(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, UnaryType_Natural, UnaryTerminator_Zero) + 1; // 1
        if (Options->Frame->BlockType == BlockType_Fixed) {
            Options->Frame->FrameNumber  = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, Bytes2Bits(Frame_SampleIDSize));
        } else if (Options->Frame->BlockType == BlockType_Variable) {
            Options->Frame->SampleNumber = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, Bytes2Bits(Frame_SampleIDSize));
        }
        
        if (Options->Frame->CodedBlockSize == 6) {
            Options->Frame->BlockSize    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 8);
        } else if (Options->Frame->CodedBlockSize == 7) {
            Options->Frame->BlockSize    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 16);
        }
        
        if (Options->Frame->CodedSampleRate == 12) {
            Options->Frame->SampleRate   = 1000 * BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 8);
        } else if (Options->Frame->CodedSampleRate == 13) {
            Options->Frame->SampleRate   = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 16);
        } else if (Options->Frame->CodedSampleRate == 14) {
            Options->Frame->SampleRate   = 10 * BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 16);
        }
    ...
    void FLAC_Parse_Subframe_LPC(FLACOptions *Options, BitBuffer *BitB, Audio2DContainer *Audio, uint8_t NumWarmupSamples) { // NumWarmupSamples=20 Max = 32
        ...
        Options->LPC->LPCPrecision = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4) + 1; // 6
        
        // Read QLP Shift
        Options->LPC->QLevel = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 5); // 20
        
        // Read Coeffs, there's NumWarmupSamples Coeffs
        for (uint8_t Coeff = 0; Coeff < NumWarmupSamples; Coeff++) {
            Options->LPC->LPCCoeff[Coeff] = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, Options->LPC->LPCPrecision);
        ...
        Options->LPC->RicePartitionType  = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 2);
    ...
    void FLAC_SubFrame_Read(FLACOptions *Options, BitBuffer *BitB, Audio2DContainer *Audio, uint8_t Channel) { // 2 channels
        ...
        AssertIO(BitB != NULL);
        AssertIO(Audio != NULL);
        bool Reserved1                             = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 1); // 0
        AssertIO(Reserved1 == 0);
        Options->Frame->Sub->SubFrameType         = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 6); // 39
        bool WastedBitsFlag                    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 1); // 0
        if (WastedBitsFlag == Yes) {
            Options->Frame->Sub->WastedBits       = BitBuffer_ReadUnary(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, UnaryType_Truncated, UnaryTerminator_One) + 1;
        }
        ...
        if (Audio2DContainer_GetType(Audio) == (AudioType_Signed | AudioType_Integer32)) {
            ...
            if (Options->Frame->Sub->SubFrameType == Subframe_Constant) {
                int32_t Constant               = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, Options->Frame->BitDepth);
                ...
                for (uint8_t Channel = 0; Channel < FLAC_GetNumChannels(Options); Channel++) {
                    for (uint16_t Sample = 0; Sample < FLAC_GetBlockSizeInSamples(Options); Sample++) {
                        Array[Channel][Sample] = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, Options->Frame->BitDepth);
                    }
                }
                ...
                uint8_t  RICEPartitionType       = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 2);
                ...
                Options->Frame->Sub->LPCPrecision   = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4) + 1; // 2 + 1?
                Options->Frame->Sub->LPCShift       = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 5); // 28
                Options->Frame->Sub->Coeff          = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, Options->Frame->Sub->LPCPrecision * Options->Frame->Sub->LPCFilterOrder); // 2 or 3 * 7 = 14 or 21 = 0xF2B or 0x
    ...
    void FLAC_Read_Residual(FLACOptions *Options, BitBuffer *BitB, Audio2DContainer *Audio) {
        ...
        AssertIO(BitB != NULL);
        AssertIO(Audio != NULL);
        uint8_t  ResiducalCodingMethod    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 2);
        uint8_t  PartitionOrder           = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4);
        ...
        if (ResiducalCodingMethod == 0) {
            RICEParameter                 = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4);
            if (RICEParameter == 15) { // Escaped
                Partition                 = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 5);
            }
        } else if (ResiducalCodingMethod == 1) {
            RICEParameter                 = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 5);
    ...
    void FLAC_Decode_RICE(FLACOptions *Options, BitBuffer *BitB, uint8_t RICEPartitionType) {
        ...
        if (RICEPartitionType == CodingType_4Bit) {
            PartitionOrder           = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4);
            ...
            if (Partitions != NULL) {
                for (uint8_t Partition = 0; Partition < NumPartitions; Partition++) {
                    Parameter    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 4);
                    if (Parameter == 15) {
                        Parameter    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 5);
                    }
                }
            }
        } else if (RICEPartitionType == CodingType_5Bit) {
            PartitionOrder           = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 5); // 17
            NumPartitions            = Exponentiate(2, PartitionOrder);
    ...
    void FLAC_Parse_StreamInfo(FLACOptions *Options, BitBuffer *BitB) {
        AssertIO(Options != NULL);
        AssertIO(BitB != NULL);
        Options->StreamInfo->MinimumBlockSize = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 16); // 4096
        Options->StreamInfo->MaximumBlockSize = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 16); // 4096
        Options->StreamInfo->MinimumFrameSize = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 24); // 18
        Options->StreamInfo->MaximumFrameSize = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 24); // 15146
        Options->StreamInfo->CodedSampleRate  = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 20); // 192000
        Options->StreamInfo->CodedChannels    = 1 + BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 3); // 2
        Options->StreamInfo->CodedBitDepth    = 1 + BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 5); // 24
        Options->StreamInfo->SamplesInStream  = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 36);    // 428,342,094
        if (Options->StreamInfo->MD5 != NULL) {
            for (uint8_t MD5Byte = 0; MD5Byte < 16; MD5Byte++) { // 0xDC AE 07 EA E9 11 40 C1 F4 43 7B B0 72 18 9D 2E
                Options->StreamInfo->MD5[MD5Byte] = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 8);
            }
        } else {
    ...
    void FLAC_Parse_SeekTable(FLACOptions *Options, BitBuffer *BitB, uint32_t ChunkSize) { // 3528
        ...
        for (uint16_t SeekPoint = 0; SeekPoint < Options->SeekPoints->NumSeekPoints; SeekPoint++) {
            Options->SeekPoints->SampleInTargetFrame[SeekPoint] = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 64);
            Options->SeekPoints->OffsetFrom1stSample[SeekPoint] = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 64);
            Options->SeekPoints->TargetFrameSize[SeekPoint]     = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 16);
        }
    }
    ...
    void FLAC_Parse_Vorbis(FLACOptions *Options, BitBuffer *BitB) { // LITTLE ENDIAN, size = 393
        AssertIO(Options != NULL);
        AssertIO(BitB != NULL);
        uint32_t VendorTagSize = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32); // 32
        UTF8    *VendorTag     = BitBuffer_ReadUTF8(BitB, VendorTagSize); // reference libFLAC 1.3.2 20170101
        uint32_t NumUserTags   = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32); // 13
        for (uint32_t Comment = 0; Comment < NumUserTags; Comment++) {
            uint32_t  TagSize                          = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32);
    ...
    void FLAC_CUE_Parse(FLACOptions *Options, BitBuffer *BitB) {
        ...
        Options->CueSheet->LeadIn    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 64);
        Options->CueSheet->IsCD      = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsLeft, 1);
        ...
        Options->CueSheet->IndexOffset    = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 64);
        Options->CueSheet->IndexPointNum  = BitBuffer_ReadBits(BitB, ByteOrder_MSByteIsLeft, BitOrder_MSBitIsRight, 8);
