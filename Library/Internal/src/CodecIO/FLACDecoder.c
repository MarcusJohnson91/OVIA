...
extern "C" {
    ...
    void FLACDecodeSubFrameLPC(BitInput *BitI, FLACFile *FLAC, uint8_t Channel) {
        ...
        if (FLAC->Data->LPC->RicePartitionType == RICE1) {
            FLACDecodeRicePartition(BitI, FLAC);
    ...
    void FLACDecodeSubFrameVerbatim(BitInput *BitI, FLACFile *FLAC) {
        ...
        for (uint8_t Partition = 0; Partition < FLAC->Data->LPC->PartitionOrder; Partition++) {
            ...
            if (RiceParameter[Partition] == 20) {
                ...
                if (FLAC->Data->LPC->PartitionOrder == 0) {
                    RiceParameter[Partition] = ReadBits(BitI, FLAC->Data->Frame->BlockSize - FLAC->Data->LPC->LPCOrder);
                } else if (FLAC->Data->LPC->PartitionOrder > 0) {
                    RiceParameter[Partition] = ReadBits(BitI, (FLAC->Data->Frame->BlockSize / pow(2, FLAC->Data->LPC->PartitionOrder)));
                } else {
                    RiceParameter[Partition] = ReadBits(BitI, (FLAC->Data->Frame->BlockSize / pow(2, FLAC->Data->LPC->PartitionOrder)) - FLAC->Data->LPC->LPCOrder);
                }
            }
        }
    }
