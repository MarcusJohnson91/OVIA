...
extern "C" {
        ...
        if (BitDepth <= 8) {
            ...
            if (ColorType == PNGColor_Palette || ColorType == PNGColor_RGB) {
                ...
                for (uint32_t Entry = 0UL; Entry < ChunkSize / 3; Entry++) {
                    Options->PLTE->Palette[0] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                    Options->PLTE->Palette[1] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                    Options->PLTE->Palette[2] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                }
            } else if (ColorType == PNGColor_RGBAlpha) {
                for (uint32_t Entry = 0UL; Entry < ChunkSize / 3; Entry++) {
                    Options->PLTE->Palette[0] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                    Options->PLTE->Palette[1] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                    Options->PLTE->Palette[2] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                    Options->PLTE->Palette[3] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsNearest, BitOrder_LSBitIsNearest, 8);
                }
            }
        }
    }
    ...
    void PNG_TRNS_Parse(PNGOptions *Options, BitBuffer *BitB, uint32_t ChunkSize) { // Transparency
        ...
        AssertIO(Entries != NULL);
        for (uint8_t Color = 0; Color < PNGNumChannelsFromColorType[Options->iHDR->ColorType]; Color++) {
            Entries[Color]    = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bits2Bytes(Options->iHDR->BitDepth, RoundingType_Up));
        }
    ...
    void PNG_BKGD_Parse(PNGOptions *Options, BitBuffer *BitB) { // Background
        ...
        for (uint8_t Entry = 0; Entry < PNGNumChannelsFromColorType[Options->iHDR->ColorType]; Entry++) {
            Options->bkGD->BackgroundPaletteEntry[Entry] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Options->iHDR->BitDepth);
        }
    }
    ...
    void PNG_PCAL_Parse(PNGOptions *Options, BitBuffer *BitB) {
        ...
        Options->pCAL->OriginalZero              = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
        Options->pCAL->OriginalMax               = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
        Options->pCAL->EquationType              = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
        Options->pCAL->NumParams                 = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
    ...
    void PNG_SBIT_Parse(PNGOptions *Options, BitBuffer *BitB) { // Significant bits per sample
        ...
        if (ColorType == PNGColor_Gray) {
            Options->sBIT->Grayscale = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
        } else if (ColorType == PNGColor_RGB || ColorType == PNGColor_Palette) {
            Options->sBIT->Red       = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
            Options->sBIT->Green     = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
            Options->sBIT->Blue      = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
            ...
            Options->sBIT->Grayscale = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
            Options->sBIT->Alpha     = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
        } else if (ColorType == PNGColor_RGBAlpha) {
            Options->sBIT->Red       = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
            Options->sBIT->Green     = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
            Options->sBIT->Blue      = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
    ...
    void PNG_HIST_Parse(PNGOptions *Options, BitBuffer *BitB) {
        AssertIO(Options != NULL);
        AssertIO(BitB != NULL);
        Options->hIST->NumColors = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
        for (uint32_t Color = 0; Color < Options->hIST->NumColors; Color++) {
            Options->hIST->Histogram[Color] = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Options->iHDR->BitDepth);
        }
    }
    ...
    void PNG_ICCP_Parse(PNGOptions *Options, BitBuffer *BitB) {
        ...
        Options->iCCP->CompressionType = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 8);
    ...
    void PNG_SPLT_Parse(PNGOptions *Options, BitBuffer *BitB, uint32_t ChunkSize) {
        ...
        for (uint32_t Entry = 0; Entry < Options->sPLT->NumEntries; Entry++) {
            Options->sPLT->Palette[Entry].Red       = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            Options->sPLT->Palette[Entry].Green     = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            Options->sPLT->Palette[Entry].Blue      = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            if (ColorType == PNGColor_RGBAlpha || ColorType == PNGColor_GrayAlpha) {
                Options->sPLT->Palette[Entry].Alpha = BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            }
        }
    }
