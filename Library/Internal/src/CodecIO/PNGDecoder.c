...
extern "C" {
    ...
    void PNG_IHDR_Read(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG           = Options;
            PNG->iHDR->Width          = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->iHDR->Height         = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
    ...
    void PNG_CHRM_Read(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG        = Options;
            PNG->cHRM->WhitePointX = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->WhitePointY = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->RedX        = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->RedY        = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->GreenX      = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->GreenY      = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->BlueX       = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->cHRM->BlueY       = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
        } else if (Options == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("Options Pointer is NULL"));
        } else if (BitB == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("BitBuffer Pointer is NULL"));
    ...
    void PNG_GAMA_Read(void *Options, BitBuffer *BitB) { // Gamma
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG  = Options;
            PNG->gAMA->Gamma = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
        } else if (Options == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("Options Pointer is NULL"));
        } else if (BitB == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("BitBuffer Pointer is NULL"));
    ...
    void PNG_OFFS_Read(void *Options, BitBuffer *BitB) { // Image Offset
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG          = Options;
            PNG->oFFs->XOffset       = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->oFFs->YOffset       = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
    ...
    void PNG_PHYS_Read(void *Options, BitBuffer *BitB) { // Aspect ratio, Physical pixel size
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG               = Options;
            PNG->pHYs->PixelsPerUnitXAxis = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->pHYs->PixelsPerUnitYAxis = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
    ...
    void PNG_PCAL_Read(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            ...
            PNG->pCAL->OriginalZero              = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->pCAL->OriginalMax               = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
    ...
    void PNG_ACTL_Read(void *Options, BitBuffer *BitB) { // Animation control, part of APNG
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG                  = Options;
            PNG->acTL->NumFrames             = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->acTL->TimesToLoop           = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32); // If 0, loop forever.
        } else if (Options == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("Options Pointer is NULL"));
        } else if (BitB == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("BitBuffer Pointer is NULL"));
    ...
    void PNG_FCTL_Read(void *Options, BitBuffer *BitB) { // Frame Control, part of APNG
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG                  = Options;
            PNG->fcTL->FrameNum              = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->fcTL->Width                 = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->fcTL->Height                = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->fcTL->XOffset               = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
            PNG->fcTL->YOffset               = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
    ...
    void PNG_HIST_Read(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            PNGOptions *PNG      = Options;
            PNG->hIST->NumColors = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
    ...
    void PNG_SPLT_Read(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            ...
            PNG->sPLT[PNG->NumSPLTChunks - 1]->Red   = (uint16_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            PNG->sPLT[PNG->NumSPLTChunks - 1]->Green = (uint16_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            PNG->sPLT[PNG->NumSPLTChunks - 1]->Blue  = (uint16_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            if (ColorType == PNG_RGBA || ColorType == PNG_GrayAlpha) {
                PNG->sPLT[PNG->NumSPLTChunks - 1]->Alpha = (uint16_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, Bytes2Bits(BitDepthInBytes));
            }
        } else if (Options == NULL) {
            Log(Severity_DEBUG, PlatformIO_FunctionName, UTF8String("Options Pointer is NULL"));
        } else if (BitB == NULL) {
    ...
    void PNG_ReadChunks(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            ...
            while (BitBuffer_GetSize(BitB) > 0) {
                uint32_t ChunkSize   = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
                uint32_t ChunkID     = (uint32_t) BitBuffer_ReadBits(BitB, ByteOrder_LSByteIsFarthest, BitOrder_LSBitIsNearest, 32);
