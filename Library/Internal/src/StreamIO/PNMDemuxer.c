...
extern "C" {
    ...
    static uint64_t PNMCheckForComment(BitBuffer *BitB) {
        ...
        if (BitBuffer_PeekBits(BitB, ByteOrder_Left2Right, BitOrder_Right2Left, 8) == PNMCommentStart) {
            BitBuffer_Seek(BitB, 8);
            do {
                CommentSize += 1;
            } while (BitBuffer_ReadBits(BitB, ByteOrder_Left2Right, BitOrder_Right2Left, 8) != PNMEndField);
        }
        return CommentSize;
    }
                ...
                for (uint64_t Height = 0; Height < Options->Height; Height++) {
                    for (uint8_t Channel = 0; Channel < Options->NumChannels; Channel++) {
                        for (uint8_t SubPixelByte = 0; SubPixelByte < 3; SubPixelByte++) {
                            Component[SubPixelByte]      = BitBuffer_ReadBits(BitB, ByteOrder_Left2Right, BitOrder_Right2Left, 8);
                        }
    ...
    void PNMExtractImage_Binary(PNMOptions *Options, BitBuffer *BitB, ImageCanvas *Image) {
        ...
        if (Options->BitDepth <= 8) {
            ...
            for (uint64_t Width = 0; Width < Options->Width; Width++) {
                for (uint64_t Height = 0; Height < Options->Height; Height++) {
                    for (uint8_t Channel = 0; Channel < Options->NumChannels; Channel++) {
                        uint8_t CurrentPixel                 = BitBuffer_ReadBits(BitB, ByteOrder_Right2Left, BitOrder_Right2Left, Options->BitDepth);
            ...
            for (uint64_t Width = 0; Width < Options->Width; Width++) {
                for (uint64_t Height = 0; Height < Options->Height; Height++) {
                    for (uint8_t Channel = 0; Channel < Options->NumChannels; Channel++) {
                        uint16_t CurrentPixel                = BitBuffer_ReadBits(BitB, ByteOrder_Right2Left, BitOrder_Right2Left, Options->BitDepth);
                        if (Options->TupleType == PNM_TUPLE_BnW) {
                            Array[0][Width][Height][Channel] = ~CurrentPixel; // 1 = black, 0 = white
                        } else {
                            Array[0][Width][Height][Channel] = CurrentPixel; // 1 = white, 0 = black
