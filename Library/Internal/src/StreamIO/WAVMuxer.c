...
extern "C" {
    ...
    static void WAVWriteFMTChunk(WAVOptions *Options, BitBuffer *BitB) {
        ...
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32, 40); // ChunkSize
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 16, 0xFFFE); // WaveFormatExtensible
        ...
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32, SampleRate * NumChannels * BitDepth);
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32, Options->BlockAlign);
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 16, BitDepth);
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 16, CBSize);
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 16, BitDepth); // ValidBitsPerSample
        BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, 32, Options->ChannelMask);
    ...
    void WAVAppendSamples(WAVOptions *Options, Audio2DContainer *Container, BitBuffer *BitB) {
        AssertIO(Options != NULL);
        AssertIO(Container != NULL);
        AssertIO(BitB != NULL);
        
        uint64_t NumChannels    = Audio2DContainer_GetNumChannels(Container);
        uint64_t BitDepth       = Audio2DContainer_GetBitDepth(Container);
        uint64_t NumSamples     = Audio2DContainer_GetNumSamples(Container);
        MediaIO_AudioTypes Type = Audio2DContainer_GetType(Container);
        
        if (Type == (AudioType_Unsigned | AudioType_Integer8)) {
            uint8_t **Samples  = (uint8_t**) Audio2DContainer_GetArray(Container);
            ...
            int8_t **Samples   = (int8_t**) Audio2DContainer_GetArray(Container);
            for (uint64_t Sample = 0; Sample < NumSamples; Sample++) {
                for (uint64_t Channel = 0; Channel < NumChannels; Channel++) {
                    BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, BitDepth, Samples[Channel][Sample]);
                }
            ...
            uint16_t **Samples = (uint16_t**) Audio2DContainer_GetArray(Container);
            for (uint64_t Sample = 0; Sample < NumSamples; Sample++) {
                for (uint64_t Channel = 0; Channel < NumChannels; Channel++) {
                    BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, BitDepth, Samples[Channel][Sample]);
                }
            ...
            int16_t **Samples  = (int16_t**) Audio2DContainer_GetArray(Container);
            for (uint64_t Sample = 0; Sample < NumSamples; Sample++) {
                for (uint64_t Channel = 0; Channel < NumChannels; Channel++) {
                    BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, BitDepth, Samples[Channel][Sample]);
                }
            ...
            uint32_t **Samples = (uint32_t**) Audio2DContainer_GetArray(Container);
            for (uint64_t Sample = 0; Sample < NumSamples; Sample++) {
                for (uint64_t Channel = 0; Channel < NumChannels; Channel++) {
                    BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, BitDepth, Samples[Channel][Sample]);
                }
            ...
            int32_t **Samples  = (int32_t**) Audio2DContainer_GetArray(Container);
            for (uint64_t Sample = 0; Sample < NumSamples; Sample++) {
                for (uint64_t Channel = 0; Channel < NumChannels; Channel++) {
                    BitBuffer_WriteBits(BitB, ByteOrder_MSByteIsRight, BitOrder_MSBitIsRight, BitDepth, Samples[Channel][Sample]);
                }
