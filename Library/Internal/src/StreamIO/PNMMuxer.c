    ...
    void PNMWriteHeader(void *Options, BitBuffer *BitB) {
        if (Options != NULL && BitB != NULL) {
            ...
            if (PNM->Type == ASCIIPNM) {
                ...
                BitBuffer_WriteBits(BitB, BitIO_ByteOrder_MSByte, BitIO_BitOrder_LSBit, 8, PNMEndField);
                free(Width);
                /* Write the Width */
                ...
                UTF8 *Width = UTF8_Integer2String(PNM->Width, Base_Integer | Base_Radix10);
                BitBuffer_WriteUTF8(BitB, Width, StringTerminator_Sized);
                BitBuffer_WriteBits(BitB, BitIO_ByteOrder_MSByte, BitIO_BitOrder_LSBit, 8, 0x0A);
                free(Width);
                /* Write the Width */
                ...
                for (uint64_t W = 0ULL; W < Width; W++) {
                    for (uint64_t H = 0ULL; H < Height; H++) {
                        for (uint16_t Channel = 0; Channel < ChannelCount; Channel++) {
                            BitBuffer_WriteBits(BitB, BitIO_ByteOrder_MSByte, BitIO_BitOrder_MSBit, ChannelCount, Array[W * H * Channel]);
                        }
                    }
                }
