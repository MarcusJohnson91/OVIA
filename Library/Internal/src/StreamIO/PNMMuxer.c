...
extern "C" {
    ...
    static void PNM_WriteWidth(BitBuffer *BitB, uint64_t Width) {
        ...
        BitBuffer_WriteBits(BitB, ByteOrder_Left2Right, BitOrder_Right2Left, 8, PNMEndField);
    ...
    static void PNM_WriteBitDepth(BitBuffer *BitB, uint64_t BitDepth) {
        ...
        BitBuffer_WriteBits(BitB, ByteOrder_Left2Right, BitOrder_Right2Left, 8, 0x0A);
            ...
            for (uint64_t W = 0ULL; W < Width; W++) {
                for (uint64_t H = 0ULL; H < Height; H++) {
                    for (uint16_t Channel = 0; Channel < ChannelCount; Channel++) {
                        BitBuffer_WriteBits(BitB, ByteOrder_Left2Right, BitOrder_Left2Right, ChannelCount, Array[W * H * Channel]);
                    }
                }
            }
